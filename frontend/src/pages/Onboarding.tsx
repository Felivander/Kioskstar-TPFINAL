...
export default function Onboarding() {
  ...
  return (
    <div className="h-screen w-screen flex items-center justify-center gradient-hero relative overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-6 lg:py-12 relative z-10 h-full overflow-hidden">
        <div className="w-full max-w-lg my-auto">
          {/* Header */}
          <div className="text-center mb-5 animate-fade-in-up" key={step}>
            <span className="text-4xl block mb-1.5 animate-float">{icon}</span>
            <h1 className="text-3xl font-extrabold text-white tracking-tight">{title}</h1>
            <p className="text-primary-200/70 mt-1 text-sm">{subtitle}</p>
          </div>

          {/* Progress bar */}
          {step !== 'choice' && step !== 'invite-code' && (
            <div className="w-full max-w-md mx-auto mb-5 animate-fade-in">
          ...
          {/* Card */}
          <div
            className="glass rounded-3xl shadow-2xl animate-scale-in max-h-[75vh] flex flex-col overflow-y-auto custom-scrollbar"
          ...
          >
            ...
            {step === 'choice' && (
              <div className="flex flex-col gap-3.5">
                <button
                  ...
                  disabled={loading}
                  className="w-full rounded-2xl border-2 border-surface-200 bg-white hover:border-primary-400 hover:bg-primary-50 transition-all duration-200 text-left group cursor-pointer"
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full rounded-2xl border-2 border-dashed border-surface-300 bg-white/50 hover:border-accent-400 hover:bg-accent-300/10 transition-all duration-200 text-left group cursor-pointer"
            ...
            {step === 'invite-code' && (
              <form onSubmit={handleInviteSubmit} className="flex flex-col gap-3.5">
                <div>
                  <label htmlFor="invite-code" className="block text-xs font-semibold text-surface-700 mb-1">
                ...
                <div className="flex gap-3 mt-1">
                  <button
                    ...
                    className="flex-1 rounded-2xl border-2 border-surface-200 text-surface-600 font-semibold text-sm hover:bg-surface-50 transition-all cursor-pointer"
                    style={{ padding: '0.75rem 1rem' }}
                  >
                    Volver
                  </button>
                  <button
                    ...
                    className="flex-1 rounded-2xl gradient-primary text-white font-semibold text-sm shadow-lg shadow-primary-500/25 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 disabled:opacity-60 flex items-center justify-center gap-2 cursor-pointer"
                    style={{ padding: '0.75rem 1rem' }}
                  >
            ...
            {/* ── STEP: Kiosk Name ── */}
            {step === 'kiosk-name' && (
              <form onSubmit={handleKioskNameNext} className="flex flex-col gap-3.5">
                <div>
                  <label htmlFor="kiosk-name" className="block text-xs font-semibold text-surface-700 mb-1">
                    Nombre del kiosco
                  </label>
                  <input
                    id="kiosk-name"
            ...
            {/* ── STEP: Kiosk Address ── */}
            {step === 'kiosk-address' && (
              <form onSubmit={handleKioskAddressNext} className="flex flex-col gap-3">
                <div>
                  <label htmlFor="kiosk-address" className="block text-xs font-semibold text-surface-700 mb-1">
                    Dirección
                  </label>
                  <input
                    id="kiosk-address"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label htmlFor="kiosk-city" className="block text-xs font-semibold text-surface-700 mb-1">
                      Ciudad
                    </label>
                    <input
                      id="kiosk-city"
                  ...
                  <div>
                    <label htmlFor="kiosk-province" className="block text-xs font-semibold text-surface-700 mb-1">
                      Provincia
                    </label>
                    <input
                      id="kiosk-province"
                ...
                <div>
                  <label htmlFor="kiosk-cp" className="block text-xs font-semibold text-surface-700 mb-1">
                    Código Postal
                  </label>
                  <input
                    id="kiosk-cp"
            ...
            {step === 'has-branches' && (
              <div className="flex flex-col gap-3.5">
                ...
                {branches.length > 0 && (
                  <div className="mb-1">
                    <p className="text-xs font-semibold text-surface-500 mb-1.5">Sucursales agregadas:</p>
                    <div className="flex flex-col gap-1.5 max-h-[140px] overflow-y-auto custom-scrollbar pr-1">
                      {branches.map((b, i) => (
                        <div key={i} className="rounded-xl bg-accent-300/10 border border-accent-400/20 text-xs text-surface-700 flex items-center gap-2" style={{ padding: '0.375rem 0.625rem' }}>
                          <span className="text-accent-500">✓</span>
                          <span className="font-bold">{b.name}</span>
                          <span className="text-surface-400">—</span>
                          <span className="text-surface-500 truncate">{b.address}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <p className="text-center text-surface-700 font-semibold text-sm">
                  {branches.length === 0
                ...
                <div className="flex gap-3 mt-1">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="flex-1 rounded-2xl border-2 border-primary-400 text-primary-600 font-semibold text-sm hover:bg-primary-50 transition-all cursor-pointer"
                    style={{ padding: '0.75rem 1rem' }}
                  >
                    Sí, agregar
                  </button>
            ...
            {/* ── STEP: Add Branch ── */}
            {step === 'add-branch' && (
              <form onSubmit={handleAddBranch} className="flex flex-col gap-3.5">
                <div>
                  <label htmlFor="branch-name" className="block text-xs font-semibold text-surface-700 mb-1">
                ...
                <div>
                  <label htmlFor="branch-address" className="block text-xs font-semibold text-surface-700 mb-1">
                    Dirección
                  </label>
                  <input
                    id="branch-address"
            ...
            {step === 'done' && (
              <div className="text-center py-2">
                ...
                <p className="text-surface-700 font-medium text-base">
                  ¡<strong>{kioskName}</strong> está listo!
                </p>
                <p className="text-surface-500 text-sm mt-1.5">Redirigiendo al panel de administración...</p>
